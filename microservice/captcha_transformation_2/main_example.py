...
if __name__ == "__main__":
    ...
    for i in range(1, len(images) + 1):
        ...
        captcha_transformation_2.v1.split_captcha.example.save_captcha(img[:a, :img.shape[1]], i)
        datasets_split_captcha.append(img[:a - 1, :img.shape[1]])
        img1 = captcha_transformation_2.v1.split_captcha.split_captcha.get_all_icons(image=img, row=a, column=b)
        icons = captcha_transformation_2.v1.split_captcha.split_captcha.split_icons(img1)
        ans = captcha_transformation_2.v1.split_captcha.split_captcha.create_icons(image=img1, icons=icons)
        for j in range(1, len(ans) + 1):
            captcha_transformation_2.v1.split_captcha.example.save_icon(index_icon=j, index_image=i, icon=ans[j - 1])
        cv2.waitKey(0)
