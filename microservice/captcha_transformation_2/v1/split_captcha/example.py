...
from captcha_transformation_2.v1.split_captcha.split_captcha import *
...
def init_dataset():
    images = []
    for i in range(1, 2):
        img = cv2.imread(f"{YOUR_PATH_DATASET}/{YOUR_NAME_CAPTCHA}_{i}.png")
        images.append(img)
    return images
...
if __name__ == "__main__":
    ...
    for i in range (1,len(images)+1):
        ...
        save_captcha(img[:a, :b],i)
        img1 = get_all_icons(image=img, row=a, column=b)
        icons = split_icons(img1)
        ans = create_icons(image=img1, icons=icons)
